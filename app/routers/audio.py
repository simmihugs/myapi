...
async def create_user(audio: CreateAudio, db: Session = Depends(get_db)):
    db_audio = db.query(AudioDB).filter(AudioDB.description == audio.description).first()
    if db_audio:
        return db_audio
    else:
...
async def query_audio(audio_description: str, db: Session = Depends(get_db)):
    db_audio = db.query(AudioDB).filter(AudioDB.description == audio_description).first()
    if db_audio is None:        
